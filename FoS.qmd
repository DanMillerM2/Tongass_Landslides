...
This section describes the infinite slope approximation, following Skempton and DeLory [-@skempton1957]. See this [article](https://nhsjs.com/2024/various-approaches-to-infinite-slope-stability-in-the-prediction-of-shallow-rain-induced-landslides/#google_vignette) in the National High School Journal of Science for a nice description of the infinite slope model.
...
@fig-FoSgradient below shows the behavior of @eq-FoS over a range of slope gradients for two contributing-area values, with all soil properties and rainfall intensity held constant. @fig-FoSsoil shows the behavior when soil depth changes. We use the DEM to estimate effects of changing gradient and contributing area on slope stability, but for now we are holding soil depth constant. 

```{r}
#| label: fig-FoSgradient
...
thisplot <- ggplot(long, aes(x=z, y=value)) + 
  ...
  labs(
    x = "Soil Depth (m)",
    y = "FoS",
    colour = "Surface Gradient",
    title = "Change of FoS with soil depth",
    subtitle = "K=1.0, b=1.0, I=0.01",
    caption = "Thinner soils and steeper slopes reduces FoS"
  ) +
  theme(legend.position = "inside", legend.position.inside = c(0.8, 0.2)) +
  scale_color_discrete(labels = c("70%", "80%"))
plot(thisplot)
```
@fig-FOS_density below shows the distribution of FoS values both within mapped landslide initiation zones and over all of Wrangell Island. This example used the contributing area calculated for a 6-hour-duraction storm. The initiation zones are distinguished by lower FoS values, indicating that this estimate of a factor of safety, based solely on topographic attributes, provides a useful means of identifying areas prone to landslide initiation.

```{r}
#| label: fig-FOS_density
library(terra)

# Get the FoS raster created with program PartialAccum for a 5-hour-duration storm.
FoS_pca6 <- terra::rast("c:/work/data/wrangell/fos_pca6.flt")
f <- as.data.table(FoS_pca6) 
# Get the FoS values at the inventoried initiation points
pnts <- terra::vect("c:/work/data/wrangell/LS_pnts.shp")
ls <- terra::extract(FoS_pca6,pnts)
# Filter the raster FoS values to include only those less than the maximum 
# point value.
#maxls <- max(ls$fos_pca6)
fsub <- subset(f, fos_pca6 <= 1.25)
f1 <- as.data.table(fsub)

p1 <- ggplot() + 
  geom_density(data=f1, aes(x=fos_pca6, fill = 'DEM'), alpha=0.5) +
  geom_density(data=ls, aes(x=fos_pca6, fill = 'Landslides'), alpha=0.5) +
  scale_fill_manual(name = "", values = c(DEM = 'blue', Landslides = 'red')) +
  labs(
    x = "Factor of Safety",
    y = "Density",
    title = "Wrangell",
    subtitle = "Storm duration = 6hr") +
  theme(legend.position = "inside", legend.position.inside = c(0.6, 0.8)) +
  coord_cartesian(x=c(0,1.2))
