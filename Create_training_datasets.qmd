...
# Creating tabular modeling datasets
...
ls_model_dataset <- function(raster_dir,shape_dir,N_bg=5000,n_ls=10){
  ...
  mask_r <- rast(paste0(raster_dir,"mask.flt"))
  ...
  all_rasters <- c(raster_files, "mask.flt","init.flt")
  raster_list <- c(as.list(rstack), list(mask_r), list(init_r))
  ...
  ls_rast   <- rasterize(slides, mask_r, field = 1, background = NA)
  cat('...masking landslide pixels...\n')
  mask_r <- mask(mask_r, ls_rast, maskvalues = 1)
  ...
  ls_vals <- extract(rstack, ls_pnt, bind = FALSE, method = ex_method)
  ...
  bg_vals <- extract(rstack, bg_coords, bind = FALSE, method = "simple")
...
wrangell <- ls_model_dataset(raster_dir,shape_dir)
head(wrangell$df)
```
...
mitkof <- ls_model_dataset(raster_dir,shape_dir)
head(mitkof$df)
```
...
cat_plots <- lapply(cat_predictors, function(pred) {
  ...
  df_filt <- df_all[df_all[[pred]] %in% keep, ]
  
  ggplot(df_filt, aes(x = .data[[pred]], fill = label)) +
