...
wrangell <- ls_model_dataset(w_raster_dir,w_shape_dir)
head(wrangell$df)
...
mitkof <- ls_model_dataset(m_raster_dir,m_shape_dir)
head(mitkof$df)
...
```{r}
wrangell$test_stack <- crop(wrangell$rstack, ext(906500,907500,492400,493400))
test_map(wrangell)
```

```{r}
mitkof$test_stack <- crop(mitkof$rstack, ext(846000, 847000, 573000, 574000))
test_map(mitkof)
```
