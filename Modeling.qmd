...
There are two things of interest, examined in the follows plots:

1.  How different, or similar, are the frequency distributions for areas inside and outside of the initiation zones?
...
for (i in 1:length(R4rasters)) {
  ...
  density[[i]] <- ggplot(den, aes(x=Upper, y=density)) + 
    geom_point(shape = 20, size = 2.2) +
...
for (i in 1:length(R4rasters)) {
  thisplot <- pbar[[i]] / pdif[[i]] / density[[i]]
...
Let's increase the density of points inside initiation zones. The previous examples had one point per initiation zone, regardless of its size. In the next example, we seek one point per every 314 m<sup>2</sup> of initiation zone area and require a 10-m buffer around points inside the initiation zones. The spacing of points outside the initiation zones is reduced to a minimum of 100 m and we still require three times as many points outside the initiation zones as inside.
...
for (i in 1:length(R4rasters)) {
  ...
  density[[i]] <- ggplot(den, aes(x=Upper, y=density)) + 
    geom_point(shape = 20, size = 3, stroke = 1.2) +
    geom_line(color = "black", linewidth = 0.9) +
    labs(x = name, y = "Density (initiation zone / unit area)") +
