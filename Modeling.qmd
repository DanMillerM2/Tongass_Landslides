...
r9 <- c("Aspect_30", "c:/work/data/wrangell/aspect_30", 0., 1.)
R4rasters <- list(r1,r2,r3,r4,r5,r6,r7,r8,r9)

# Add landform type
i1 <- c("Landform", "c:/work/data/wrangell/newLandform")
i2 <- c("Soils", "c:/work/data/wrangell/soils_pm")
...
corMatrix <- layerCor(rstack, fun = "pearson", na.rm = TRUE)
```
...
#for (i in 2:3) {
  ...
  roc <- roc[, tpr := rev(cumsum(Number))]
  tprmax <- max(roc$tpr)
  roc <- roc[, tpr := tpr/tprmax]
  roc <- roc[, fpr := rev(cumsum(Nout))]
  fprmax <- max(roc$fpr)
  ...
  xrange <- list(min(den$Upper), max(den$Upper))
  ...
  density[[i]] <- ggplot(data=den) + 
    ...
    coord_cartesian(xlim=c(xrange[[1]],xrange[[2]]), ylim=c(0.,0.007))
...
```{r}
simple <- data_wide[, .(Class, FoS_72, Aspect_30, Landform, Soils)]
...
tsk_init_gam <- as_task_classif_st(simpl, target = "Class", positive = "Landslide")
lrn_gam <- lrn("classif.gam", predict_type = "prob")
lrn_gam$param_set$set_values(
  formula = Class ~ s(FoS_72, k=4) + s(Aspect_30, k=4))

lrn_gam$train(tsk_init_gam)
